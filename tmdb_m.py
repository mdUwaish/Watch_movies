...
def get_genre_movies(genre_id):
    ...
    for i in range(0, len(list_genre)):
        ...
        arr = []
        arr.append(list_genre[i].get('title', ''))
        arr.append(list_genre[i].get('id', ''))
        arr.append(list_genre[i].get('release_date', ''))
        arr.append(list_genre[i].get('vote_average', ''))
        # print(list_genre, i)
        url = "https://www.themoviedb.org/movie/"+str(list_genre[i].get('id',''))+"/watch"
        path = "https://image.tmdb.org/t/p/original"+ list_genre[i].get('poster_path','')
        arr.append(url)
        arr.append(path)
        arr.append(list_genre[i].get('vote_count',''))
        arr.append(list_genre[i].get('overview',''))
        backdrop = "https://image.tmdb.org/t/p/original" + str(list_genre[i].get('backdrop_path',''))
        arr.append(backdrop)
        arr.append(list_genre[i].get('popularity',''))
        movies.append(arr)
    return movies
